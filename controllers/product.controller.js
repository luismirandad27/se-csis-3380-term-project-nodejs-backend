...
exports.getProductByProdId = async (req, res) => {
    ...
    const product = await Product.findOne({
        ...
        .populate("product_category")
        .populate({
            path: "product_subtypes.reviews.user",
            select: "company logo_image_url created_at" // Assuming the field is called companyName, adjust as needed
        });
    ...
    product.product_subtypes.forEach(subtype => {
        const latestReviewsMap = new Map();

        // Assuming reviews are already sorted by created_at in descending order
        // If not, you might need to sort them before this step
        subtype.reviews.forEach(review => {
            const userId = review.user._id.toString();
            if (!latestReviewsMap.has(userId)) {
                latestReviewsMap.set(userId, review);
            }
        });

        // Convert the Map values back to an array
        subtype.reviews = Array.from(latestReviewsMap.values());
    });
...
const constructFilters = async (query) => {
    ...
    if (query.category) {
        ...
        if (categories.length) {
            generalCondition.product_category = {
    ...
    if (query.grind) {
        ...
        const grinds = await GrindType.find({
            name: {
                $in: grindNames
            }
        }, '_id');
        if (grinds.length) {
            generalCondition.grind_types = {
                $in: grinds.map(grind => grind._id)
            };
        }
    }
    ...
    if (query.weight) {
        ...
        const weights = await WeightType.find({
            name: weightValue
        }, '_id');
        if (weights.length) {
            subtypeCondition["product_subtypes.weight._id"] = {
                $in: weights.map(weight => weight._id)
            };
        }
    }
    ...
    // Price Filter
    if (query.min) {
        subtypeCondition["product_subtypes.price"] = {
...
const getPipeline = (generalCondition, subtypeCondition, sortCondition) => {
    let pipeline = [{ //We filter based on the main fields (product_category, grind_types, countries_origin)
        ...
        { // We group the documents back together, using the _id as the grouping key
            $group: {
                ...
                name: {
                    $first: "$name"
                }, // Take the first encountered name value
                prod_id: {
                    $first: "$prod_id"
                },
                description: {
                ...
                import_partner: {
                    $first: "$import_partner"
                },
                grind_types: {
                    $first: "$grind_types"
                },
                product_subtypes: {
                    $push: "$product_subtypes"
                }, // Reconstruct the product_subtypes array
            }
        },
        {
            $addFields: {
                first_subtype_price: {
                    $arrayElemAt: ["$product_subtypes.price", 0]
                }
            }
        },
...
exports.getAllProducts = async (req, res) => {
    try {
        ...
        const {
            pageSize,
            page
        } = getPaginationInfo(req.query); // Pagination
        const {
            generalCondition,
            subtypeCondition,
            sortCondition
        } = await constructFilters(query); // Filters
        ...
        const mainPipeline = [...basePipeline, {
            $skip: (page - 1) * pageSize
        }, {
            $limit: pageSize
        }];
